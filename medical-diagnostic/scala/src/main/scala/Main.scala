...
object MedicalDiagnosticServer {
  ...
  def callPythonDiagnose(symptomsJson: String): String = {
    try {
      ...
      // Leer respuesta
      val response = scala.io.Source.fromInputStream(
        ...
        function displayResults(data) {
            ...
            const diagnosisHtml = diagnosisList.map(diagnosis => `
                <div class="diagnosis-card">
                    ...
                    <p class="diagnosis-description">${diagnosis.descripción}</p>
                    <span class="confidence">Confianza: ${Math.round(diagnosis.confianza * 100)}%</span>
                </div>
            `).join('');
